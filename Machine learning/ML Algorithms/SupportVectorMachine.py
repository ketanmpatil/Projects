...
class SVM():
    def __init__(self,lambad_param=0.01,learning_rt=0.001,n_iters=1000):
        self.lambda_param = lambad_param
    ...
    def fit(self,X,y):
        ...
        for _ in range(self.n_iters):
            for idx,val in enumerate(X):
                function = np.dot(y_[idx],np.dot(val,self.weights)-self.bias) >= 1
                if function:
                    self.weights -= self.learning_rt * (2*np.dot(self.lambda_param,self.weights))
                else:
                    self.weights -= self.learning_rt * (2 * self.lambda_param * self.weights - np.dot(val, y_[idx]))
                    self.bias -= self.learning_rt * y_[idx]
